...
interface FileSystemDirectoryHandle { kind: "directory"; name: string; }
...
function App() {
  ...
  async function uploadBatch(items: DirectoryFile[], label: string) { if (!selectedProject || !items.length) return; setFolderName(`${label}（${items.length} 个文件）`); setUploading(true); try { const BATCH_SIZE = 20; for (let i = 0; i < items.length; i += BATCH_SIZE) { const chunk = items.slice(i, i + BATCH_SIZE); const data = new FormData(); chunk.forEach(item => { data.append("files", item.file, item.file.name); data.append("relative_paths", item.relativePath); }); const r = await fetch(`${API}/api/projects/${selectedProject}/files`, { method: "POST", body: data }); if (!r.ok) throw new Error(`Upload failed (${r.status})`); } } finally { setUploading(false); } }
  const SKIP_DIRS = new Set([".git", "node_modules", "__pycache__", ".venv", "venv", "dist", "build", ".next", "coverage", ".idea", ".vscode", ".tox", ".mypy_cache", ".pytest_cache"]);
  const SKIP_EXTENSIONS = new Set([".png", ".jpg", ".jpeg", ".gif", ".ico", ".woff", ".woff2", ".ttf", ".eot", ".mp4", ".mp3", ".zip", ".tar", ".gz", ".exe", ".dll", ".so", ".dylib", ".class", ".jar", ".war", ".pdf", ".lock"]);
  const MAX_FILE_SIZE = 1024 * 1024; // 1MB
  async function collectDirectory(handle: FileSystemDirectoryHandle, prefix = ""): Promise<DirectoryFile[]> { const result: DirectoryFile[] = []; for await (const entry of handle.values()) { if (entry.name.startsWith(".") && entry.name !== ".env.example") continue; const relative = prefix ? `${prefix}/${entry.name}` : entry.name; if (entry.kind === "file") { const ext = entry.name.includes(".") ? "." + entry.name.split(".").pop()!.toLowerCase() : ""; if (SKIP_EXTENSIONS.has(ext)) continue; const file = await entry.getFile(); if (file.size > MAX_FILE_SIZE) continue; result.push({ file, relativePath: relative }); } else { if (SKIP_DIRS.has(entry.name)) continue; result.push(...await collectDirectory(entry, relative)); } } return result; }
  async function chooseFolder() { if (!selectedProject || uploading) return; if (window.showDirectoryPicker) { try { const handle = await window.showDirectoryPicker({ mode: "read" }); await uploadBatch(await collectDirectory(handle), handle.name); return; } catch (error) { if (error instanceof DOMException && error.name === "AbortError") return; } } document.getElementById("folder-upload-fallback")?.click(); }
  async function uploadFolder(event: React.ChangeEvent<HTMLInputElement>) { if (!event.target.files?.length) return; const items = Array.from(event.target.files).filter(file => { const rel = file.webkitRelativePath || file.name; const parts = rel.split("/"); if (parts.some(p => SKIP_DIRS.has(p))) return false; if (parts.some(p => p.startsWith(".") && p !== ".env.example")) return false; const ext = file.name.includes(".") ? "." + file.name.split(".").pop()!.toLowerCase() : ""; if (SKIP_EXTENSIONS.has(ext)) return false; if (file.size > MAX_FILE_SIZE) return false; return true; }).map(file => { const rel = file.webkitRelativePath || file.name; const parts = rel.split("/"); return { file, relativePath: parts.slice(1).join("/") || file.name }; }); if (!items.length) { event.target.value = ""; return; } await uploadBatch(items, (Array.from(event.target.files)[0].webkitRelativePath || "").split("/")[0] || "project"); event.target.value = ""; }
  async function uploadFiles(event: React.ChangeEvent<HTMLInputElement>) { if (!selectedProject || !event.target.files?.length) return; setUploading(true); const data = new FormData(); Array.from(event.target.files).forEach(file => { data.append("files", file); data.append("relative_paths", file.name); }); try { const r = await fetch(`${API}/api/projects/${selectedProject}/files`, { method: "POST", body: data }); if (!r.ok) throw new Error("Upload failed"); setFolderName(`${event.target.files.length} 个文件`); } finally { setUploading(false); event.target.value = ""; } }
  async function submit(event: FormEvent) { event.preventDefault(); if (!prompt.trim() || running || !selectedProject || !selectedSession) return; const value = prompt.trim(); setPrompt(""); setRunning(true); setEvents(x => [...x, { type: "user", content: value }]); try { const r = await fetch(`${API}/api/runs`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: value, project_id: selectedProject, session_id: selectedSession }) }); if (!r.ok) throw new Error(`Request failed (${r.status})`); const reader = r.body?.getReader(); if (!reader) throw new Error("Streaming unavailable"); const decoder = new TextDecoder(); let buffer = ""; while (true) { const result = await reader.read(); if (result.done) break; buffer += decoder.decode(result.value, { stream: true }); const chunks = buffer.split("\n\n"); buffer = chunks.pop() ?? ""; for (const chunk of chunks) { const line = chunk.split("\n").find(x => x.startsWith("data: ")); if (line) { const item = JSON.parse(line.slice(6)); setEvents(x => [...x, item]); if (item.type === "approval.required") setApproval(item); if (item.type === "plan.created") setPlan({ id: item.plan_id, summary: item.content, status: "WAITING_CONFIRMATION" }); if (item.type === "plan.confirmation_required") setPlan(p => p ? { ...p, goal: item.content } : p); } } } } catch (error) { setEvents(x => [...x, { type: "run.failed", content: error instanceof Error ? error.message : "Request failed" }]); } finally { setRunning(false); } }
  async function loadPlanTasks(planId: number) { const r = await fetch(`${API}/api/plans/${planId}/tasks`); if (r.ok) setPlanTasks(await r.json()); }
  async function pollPlanTasks(planId: number) {
    for (let i = 0; i < 120; i++) {
      ...
      if (updated.status === "SUCCEEDED" || updated.status === "FAILED" || updated.status === "CANCELLED" || updated.status === "PAUSED") break;
    }
  }
  async function planAction(action: "confirm" | "cancel" | "pause" | "resume") { if (!plan) return; const r = await fetch(`${API}/api/plans/${plan.id}/${action}`, { method: "POST" }); if (!r.ok) { setEvents(x => [...x, { type: "plan.failed", content: `计划操作失败 (${r.status})` }]); return; } const updated = await r.json(); setPlan(updated); await loadPlanTasks(plan.id); if (action === "confirm" || action === "resume") { setEvents(x => [...x, { type: "plan.started", content: `计划已${action === "confirm" ? "确认并开始执行" : "继续执行"}` }]); pollPlanTasks(plan.id); } }
