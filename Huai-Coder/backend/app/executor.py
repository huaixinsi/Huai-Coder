...
SCHEDULABLE = {"PENDING", "READY", "RETRYING", "BLOCKED"}
...
async def next_task(db: AsyncSession, plan_id: int) -> Task | None:
    tasks = list(
        (
            await db.scalars(
                select(Task).where(Task.plan_id == plan_id).order_by(Task.id)
            )
        ).all()
    )
    for task in tasks:
        ...
        if all(dep and dep.status == "SUCCEEDED" for dep in states):
            return task
...
async def mark_task_success(db: AsyncSession, task: Task, output: str):
    ...
    task.output_data = output[:12000]
    task.finished_at = now()
    await db.commit()


async def mark_task_failure(
    db: AsyncSession, task: Task, error_type: str, message: str
):
    task.error_type = error_type
    task.error_message = message[:4000]
    task.retry_count += 1
    if task.retry_count <= task.max_retries and error_type in {
        "LLM_ERROR",
        "TOOL_ERROR",
        "COMMAND_TIMEOUT",
    }:
