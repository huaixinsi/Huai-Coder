...
| 能力 | 说明 |
| --- | --- |
| 项目工作区 | 为每个项目隔离文件和会话，支持绑定/切换完整文件夹；切换时会替换当前项目工作区，避免旧项目文件残留。 |
...
2. 创建或选择会话，先绑定对应的完整项目目录；切换项目目录时点击“绑定/切换文件夹”。
3. 确认当前会话顶部显示的工作区后再输入任务。未绑定文件夹时发送按钮会被阻止并提示绑定。
4. 检查计划和任务依赖，点击“确认计划”。
5. Agent 可通过 `write_file` 创建或修改代码，并建议用 `read_file` 或测试命令验证结果。
6. 高风险审批由 `TOOL_APPROVAL_ENABLED` 控制，当前默认关闭；开启后可在审批弹窗中批准、拒绝或取消。
7. 在“执行过程”面板中展开任意工具调用，查看具体参数、返回结果和重复调用防护事件。
8. 在项目面板中维护长期记忆，或点击“压缩会话”生成可追溯的会话摘要。
